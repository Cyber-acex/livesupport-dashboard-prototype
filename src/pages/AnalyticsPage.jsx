...
function normalizeMonthlyLabels(labels = [], aiData = [], staffData = []) {
  ...
  return labels
    .map((label, index) => ({ label, ai: aiData[index] || 0, staff: staffData[index] || 0, order: monthOrder[label] ?? 999 }))
    .sort((a, b) => a.order - b.order);
}

function createLineChart(ctx, labels, aiData, staffData) {
  ...
  return new window.Chart(ctx, {
    ...
    options: {
      ...
      interaction: { mode: 'nearest', intersect: false, axis: 'xy', radius: 30 },
      scales: {
        x: { grid: { display: false }, ticks: { color: '#333' } },
        y: { beginAtZero: true, ticks: { color: '#333' }, grid: { color: 'rgba(148,163,184,0.18)', borderDash: [4, 4] } }
      }
    }
  });
}
...
function AnalyticsPage() {
  ...
  useEffect(() => {
    if (window.Chart && aiStaffRef.current) {
      const normalized = normalizeMonthlyLabels(monthlyMessages.labels, monthlyMessages.ai, monthlyMessages.staff);
      const chart = aiStaffChart || createLineChart(aiStaffRef.current.getContext('2d'), normalized.map((i) => i.label), normalized.map((i) => i.ai), normalized.map((i) => i.staff));
      if (!aiStaffChart) setAiStaffChart(chart);
    }
  }, [monthlyMessages, aiStaffChart]);
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3 rounded-full bg-slate-100 p-2 shadow-sm">
              {['analytics', 'staff'].map((tab) => (
                <button
                  key={tab}
                  type="button"
                  className={`rounded-full px-4 py-2 text-sm font-semibold transition ${activeTab === tab ? 'bg-slate-900 text-white shadow-lg' : 'bg-transparent text-slate-700 hover:bg-slate-200'}`}
            ...
            {activeTab === 'analytics' ? (
              <section className="space-y-6">
                <div className="grid gap-4 lg:grid-cols-[minmax(220px,1fr)_minmax(220px,1fr)_minmax(220px,1fr)_minmax(220px,1fr)]">
                  <div className="rounded-2xl bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.07)]">
                  ...
                  <div className="flex flex-col gap-3 rounded-2xl bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.07)]">
                    <button type="button" onClick={() => {}} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white hover:bg-slate-800">Apply</button>
                    <button type="button" onClick={() => {}} className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-200">Export CSV</button>
                  </div>
                </div>
              ...
              <section className="space-y-6">
                ...
                <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.07)]">
                    ...
                    {presenceFiltered.length === 0 ? (
                      ...
                      <div key={agent.userId} className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_20px_40px_rgba(15,23,42,0.08)]">
                        <div className="flex items-center gap-4">
                          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-indigo-600 text-white text-lg font-bold">{agent.name?.charAt(0) || 'A'}</div>
                          <div>
                ...
                <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
                  <aside className="rounded-[28px] bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.07)]">
                    <div className="mb-6">
                  ...
                  <div className="rounded-[28px] bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.07)]">
                    ...
                    <div className="grid gap-3 rounded-3xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                      <div className="grid grid-cols-[1.7fr_0.9fr_0.85fr_0.85fr_0.85fr_1.05fr_0.8fr] gap-3 px-3 py-2 font-semibold text-slate-600 uppercase tracking-[0.1em]"> 
