...
function renderUsers(users) {
  ...
  users.forEach(u => {
    ...
    tr.innerHTML = `<td>${u.id}</td><td class="staffNameCell">${escapeHtml(u.name)}</td><td>${escapeHtml(u.email)}</td><td><select data-id="${u.id}" class="roleSel"><option value="agent">agent</option><option value="admin">admin</option><option value="viewer">viewer</option><option value="Delivery Support">Delivery Support</option><option value="Refund Manager">Refund Manager</option><option value="Kitchen Supervisor">Kitchen Supervisor</option><option value="Customer Support">Customer Support</option></select></td><td>${u.active ? '<span style="display:inline-flex;align-items:center;gap:6px;"><span style="width:10px;height:10px;background:#10b981;border-radius:50%;display:inline-block;"></span><small style="color:#065f46">Active</small></span>' : '<small style="color:#6b7280">Offline</small>'}</td><td><input type="checkbox" data-id="${u.id}" class="disabledChk" ${u.disabled ? 'checked' : ''} /></td><td></td>`;
