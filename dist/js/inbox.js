...
// Start polling if socket doesn't connect within 5 seconds
socketConnectTimeout = setTimeout(() => {
    if (!socketConnected) {
        console.warn('[Socket.IO] Connection timeout - enabling fallback polling');
        startMessagePolling();
    }
}, 5000);
...
socket.on('connect', () => {
    ...
    // Clear polling interval when socket connects
...
socket.on('connect_error', (error) => {
    ...
    socketConnected = false;
    if (!messagePollingInterval) {
        console.warn('[Socket.IO] Starting polling due to connection error');
        startMessagePolling();
    }
});
...
function startMessagePolling() {
    ...
    messagePollingInterval = setInterval(async () => {
        ...
        try {
            // Always refresh conversations list to catch new messages
            const activeFilter = getActiveInboxFilter();
            await loadConversations(activeFilter);
            
            // Poll for messages in current conversation if one is open
            if (currentConversationId) {
                ...
                if (Array.isArray(messages) && messages.length > 0) {
                    ...
                    if (new Date(lastMsg.created_at).getTime() > lastMessageTimestamp) {
                        console.log('[Polling] New message detected in current conversation, updating...');
